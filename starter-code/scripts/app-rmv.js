...
function init() {
  ...
  const width = 12
  //const clickedGrid = squares[indexOfClickedGrid()]
  let userIndex = null //?
  let timerID = null
  ...
  const grids = document.querySelectorAll('.grid-item') //? similar to board
  ...
  // const bombCounterDisplay = document.querySelectorAll('.grid-item') //?
  let bombCount = parseInt(bombCounterDisplay.textContent) //?
  let flagCount = document.querySelector('.flag-number').innerHTML //?
  ...
  function clicked() {
    if (!gameInPlay) {//? BEFORE GAME STARTED
      ...
      clickedGrid.addEventListener('click', e => {
        e.target.classList.add('first-click', 'empty-grid')
      })
      ...
      if (bombCount === 0) {
        ...
        clickedNonEmpty()
      } else if (clickedGrid.classList.contains('bomb')) {
        explode()
        finishGame()
      } 
    }
  ...
  function clickedNonEmpty() {
    clickedGrid.textContent = bombCount
    clickedGrid.addEventListener('click', e => {
  ...
  function explode() {
    //TODO exlosion scene
    alert('oh no! you clicked on a bomb!')
  }
  ...
  function generateRandomNums(min, max) {
    ...
    } //TODO check `clickedGrid` (changed game logic)
  ...
  function resetBoard() {
    ...
    window.location.reload() //TODO LAZY WAY OF RESETTING...
  }
